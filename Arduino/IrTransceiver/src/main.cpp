...
#include <WiFi.h>
#define IR_RECV_PORT 1
#define IR_SEND_PORT 2
...
#define UDP_REMOTE_ADDRESS 0xC0A80110 /* Node.jsサーバのIPアドレス(192.168.1.16の場合) */
...
static long udpSend(JsonDocument& jdoc, uint32_t ipaddress, uint16_t port);
...
static long process_button_pressed(char btn);
...
void loop() {
  ...
  Btn.read();
  if( Btn.wasPressed() ){
    long ret = process_button_pressed(KEY_ID_1);
    if( ret != 0 )
      Serial.println("process_button_pressed Error");
  }
#elif defined(ARDUINO_ESP32S3_DEV)
  Btn.read();
  if( Btn.wasPressed() ){
...
static long process_button_pressed(char btn){
  Serial.println("process_button_pressed");

  jsonDoc.clear();
  jsonDoc["type"] = "button_pressed";
  jsonDoc["button_type"] = btn;

  long ret = udpSend(jsonDoc, UDP_REMOTE_ADDRESS, UDP_REMOTE_PORT );
  if( ret != 0 ){
...
static long process_udp_receive(int packetSize)
{
  ...
  if( strcmp(cmd_type, "ir_send") == 0 ){
    ...
    irrecv.resume();
    if( ret != 0 ){
      Serial.println("can't irRend");
      return ret;
    }
  }else{
    Serial.println("Unknown cmd_type");
    free(p_buffer);
    return -1;
  }
...
static long udpSend(JsonDocument& jdoc, uint32_t ipaddress, uint16_t port)
{
  ...
  udp.beginPacket(IPAddress((ipaddress >> 24) & 0xff, (ipaddress >> 16) & 0xff, (ipaddress >> 8) & 0xff, (ipaddress >> 0) & 0xff), port);
